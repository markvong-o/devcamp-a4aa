...
- All Dashboard steps from *One trust boundary for every agent*, *Every agent action has an owner*, *The agent acts as the employee, not a shared bot*, and *Humans approve what can't be undone* are completed (Token Exchange enabled, Token Vault enabled on the CRM connection, Auth0 My Account API activated and the SPA authorized for its Connected Accounts scopes, CIBA enabled at the tenant level).
- Codespace port 3002 (CRM mock) is set to **Public** visibility (see *The agent acts as the employee, not a shared bot*), required for the CRM's OAuth redirect to complete.
