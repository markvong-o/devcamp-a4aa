...
This lab consists of **five (5)** core modules, each adding one security layer to the Nexus MCP server deployment. Your Auth0 tenant is provisioned for you on launch. 

Most modules are hands-on. One, Fine-Grained Authorization, runs as a live demo that illustrates enforcement in action.

Here is what we will get into:

1. **One trust boundary for every agent**: ***Auth for MCP***

   *Register the MCP server as an Auth0 resource and publish Protected Resource Metadata (PRM) plus Authorization Server (AS) discovery so any compliant client finds it with zero configuration. Pre-register the first-party Nexus agent via CIMD so it has a stable, auditable identity across deploys. OBO token exchange carries the employee's **sub** through the agent boundary, ensuring every tool call downstream identifies exactly who triggered it.*

2. **Every agent action has an owner**: ***User Authentication***

   *The MCP server now identifies its callers, but before it can enforce policy downstream, it needs a verified employee in the session. Wire Universal Login so every request carries the JWT **sub** that the On-Behalf-Of token exchange will preserve to the tool, establishing the user context for all downstream authorization decisions.*
